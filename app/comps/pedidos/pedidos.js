          ...
          if( $scope.clienteEmpleado == true){
            ...
            if(articulo.tipo_prod_emp == "MEDICINA"){
              ...
              if( ($scope.totales.empMed + articulo.CANTIDAD) > $scope.client.unid_disp_med_emp){
                notify({ message:'¡La cantidad ingresada excede la cantidad que usted tiene disponible ( <strong>'+($scope.totales.empMed - $scope.client.unid_disp_med_emp )+' </strong>) para productos misceláneos!', position:'right', duration:10000, classes:'alert-danger'});
               return  true;
              }
